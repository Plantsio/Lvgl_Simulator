...
namespace UI {
    ...
    city_label(lay_left){
		...
		THEME_SET_FONT_SIZE(current_temp_label,120);
		...
		THEME_SET_FONT_SIZE(temp_range_label,30);

        temp_unit_label = lv_label_create(m_scr);
		THEME_SET_FONT_SIZE(temp_unit_label,30);
    ...
    static std::string get_temp_abs() {
        return get_degree_string(SysOld::get_geo_info().weather.temp);
    }

    static std::string get_temp_max() {
        return get_degree_string(SysOld::get_geo_info().weather.max);
    }

    static std::string get_temp_min() {
        return get_degree_string(SysOld::get_geo_info().weather.min);
    }
    ...
    void UIWeather::update() {
        ...
        if (Prop::get<int>(Prop::language) != Lang::zh)
        {
            city_label.label_set_text(SysOld::get_geo_info().location.city_district.c_str());
            weather_label.label_set_text(SysOld::get_weather_by_lang((Lang::lang_t)Prop::get<int>(Prop::language)).c_str());
        }
        ...
        lv_obj_align_to(temp_unit_label, current_temp_label, LV_ALIGN_OUT_RIGHT_TOP, -15, -10);
